...
def gcg_step_multi(input_sequence_list, adv_idxs_list, model_list, loss_function, forbidden_tokens, top_k, num_samples, batch_size, top_candidates):
    ...
    Args:
        ...
        batch_size: Batch size for the attack.
        adv_idxs: List of indices of adversarial tokens in the prompt.
        top_candidates: Number of top adversarial sequences with minimum loss to be used for multi-coordinate updates.
            Must be less than or equal to batch_size.
    Returns:
        input_sequence: Updated input sequence.
        min_loss: Minimum loss of the batch of adversarial sequences generated by the attack.
    """

    assert top_candidates <= batch_size, "top_candidates must be less than or equal to batch_size."
    ...
    top_candidate_list = [[] for _ in range(num_models)]     # List to store top candidates with minimum loss
    min_loss_list = []
    ...
    for i in range(ceil(num_samples / batch_size)):
        ...
        # Find the top candidates with minimum loss
        min_batch_loss, min_loss_indices = torch.topk(batch_loss, this_batch_top_candidates, largest=False)

        # Update top candidates list by merging with the previous top candidates
        ...
        for top_cand_idx in range(top_candidates):
            if this_batch_top_cand_idx < this_batch_top_candidates:
                if top_cand_idx >= len(min_loss_list) or min_batch_loss[this_batch_top_cand_idx] < min_loss_list[top_cand_idx]:
                    # Insert new top candidate in the list of top candidates
                    for j in range(num_models):
        ...
        # Trim the list of top candidates and minimum losses
        top_candidate_list = [top_candidate_list[j][:top_candidates] for j in range(num_models)]
    ...
    # Multi-coordinate update
    for model_idx in range(num_models):
        for top_cand_idx in range(1, top_candidates):
            for token_idx in range(len(adv_idxs_list[model_idx])):
                if top_candidate_list[model_idx][top_cand_idx][0, adv_idxs_list[model_idx][token_idx]] == input_sequence_list[model_idx][0, adv_idxs_list[model_idx][token_idx]]:
    ...
    top_candidate_list = [torch.cat(top_candidate, dim=0) for top_candidate in top_candidate_list]

    # Compute loss for the top candidates
    ...
    # Check if tokens at adversarial indices are the same in all models
    if adv_seq_list is not None:
        for j in range(1, num_models):
            # print(adv_seq_list[j][0, adv_idxs_list[j]])
            if not torch.all(adv_seq_list[0][0, adv_idxs_list[0]] == adv_seq_list[j][0, adv_idxs_list[j]]):
                print("Adversarial tokens at adversarial indices are not the same in all models.")
                break
